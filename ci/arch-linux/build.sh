...
echo "::group::Prepare"
set -x
cp /source/groonga-normalizer-mysql-*.tar.gz ./
version=$(echo groonga-normalizer-mysql-*.tar.gz | grep -E -o '[0-9]+\.[0-9]+\.[0-9]+')
touch groonga-normalizer-mysql-${version}.tar.gz.asc
sed \
...
pushd groonga
# TODO: Remove this once Groonga 15.2.2 is released:
sed \
  -i"" \
  -e "s/GRN_WITH_MRUBY=ON/GRN_WITH_MRUBY=ON -D GRN_WITH_XSIMD=bundled/" \
  PKGBUILD
...
sudo pacman \
  ...
  make \
  ruby-erb
rm -rf groonga-normalizer-mysql-${version}
tar xf groonga-normalizer-mysql-${version}.tar.gz
(
  cd groonga-normalizer-mysql-${version}
